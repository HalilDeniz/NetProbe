...
class NetProbe:
    ...
    def scan(self, target, iface):
        ...
        try:
            ...
            self.console.print("[bold red]Error:[/] You do not have sufficient privileges. Try running the program with 'sudo'.")
            exit()
        except OSError as e:
            if "No such device" in str(e):
                self.console.print(f"[bold red]Error:[/] Interface '{iface}' does not exist. \nPlease provide a valid interface name.")
                exit()
            else:
                raise
        ...
        for sent, received in result:
            ...
            manufacturer = self.get_device_info(mac_address)
    ...
    def apply_filters(self, devices, manufacturer_filter, ip_range_filter):
        filtered_devices = []
        for device in devices:
            if (not manufacturer_filter or device['manufacturer'].lower() == manufacturer_filter.lower()) and (not ip_range_filter or self.ip_in_range(device['ip'], ip_range_filter)):
    ...
    def display_live_updates(self, targets, interfaces, output_file=None, manufacturer_filter=None, ip_range_filter=None, scan_rate=5):
        known_devices = []
        while True:
            for target in targets:
                for iface in interfaces:
                    devices = self.scan(target, iface)

                    # Save newly discovered devices
                    new_devices = [d for d in devices if d not in known_devices]
                    if new_devices and output_file:
                        self.save_results(new_devices, output_file)
                        known_devices.extend(new_devices)

                    filtered_devices = self.apply_filters(devices, manufacturer_filter, ip_range_filter)
    ...
    def save_results(self, devices, output_file):
    ...
    def main(self):
        try:
            ...
            for target in targets:
                for iface in interfaces:
                    devices.extend(self.scan(target, iface))
            ...
            if output_file:
                self.save_results(filtered_devices, output_file)
                self.console.print(f"\n[bold green]Results saved to: {output_file}")

            if live_tracking:
                t = threading.Thread(target=self.display_live_updates, args=(targets, interfaces, output_file, manufacturer_filter, ip_range_filter, scan_rate))
